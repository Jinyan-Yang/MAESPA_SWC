    ...
    SUBROUTINE PSTRANSPIF(iday,ihour,RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,CA,RH,VPD,VMFD,PRESS,JMAX25,&
        ...
        VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,IJV,RD,ET,FHEAT, &
    ...
    INTEGER IECO,ITERMAX,NSIDES,VFUN,IJV
    integer iday,ihour
    ...
    IF(MODELGS.EQ.6)THEN
        ...
        CALL PSILFIND(RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,CA,RH,VPD,VMFD,PRESS,JMAX25,&
            ...
            Q10F,K10F,RTEMP,DAYRESP,TBELOW,MODELGS,WSOILMETHOD,EMAXLEAF,SOILMOISTURE,    &
            SMD1,SMD2,WC1,WC2,SOILDATA,SWPEXP,FSOIL,G0,D0L,GAMMA,VPDMIN,G1,GK,WLEAF,NSIDES,   &
            VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,IJV,RD,ET,FHEAT,TLEAF,GBH,PLANTK,TOTSOILRES,MINLEAFWP,  &
...
100 CONTINUE  ! Return point for iterations
    ...
    CALL PHOTOSYN(PAR,TLEAF,TMOVE,CS,RHLEAF,DLEAF,VMLEAF,JMAX25,IECO,EAVJ,EDVJ,DELSJ,VCMAX25,&
        ...
        G0,D0L,GAMMA,VPDMIN,G1,GK,GSC,ALEAF,IJV,RD,MINLEAFWP,KTOT,WEIGHTEDSWP, &
...
200 FHEAT = RNET - LHV*ET
    ...
    SUBROUTINE PHOTOSYN(PAR,TLEAF,TMOVE,CS,RH,VPD,VMFD, &
        ...
        SOILDATA,SWPEXP, &
        FSOIL,G0,D0L,GAMMA,VPDMIN,G1,GK,GS,ALEAF,IJV,RD,MINLEAFWP,KTOT,WEIGHTEDSWP, &
        VPARA,VPARB,VPARC,VFUN,SF,PSIV,HMSHAPE,PSILIN,PSIL,CI,ISMAESPA)
    ...
    ! BM 02/19 Indicate whether photosynthesis is J-limited (0) or V-limited (1)
    IJV = 0

    ! Deal with extreme cases
    IF ((JMAX.LE.0.0).OR.(VCMAX.LE.0.0)) THEN
        ALEAF = -RD
        IJV = -1 ! BM 02/19 Set to -1 to indicate no photosyn calculation
        GS = G0
    ...
    FSOIL = CALCFSOIL(WEIGHTEDSWP,SWPEXP)
    ...
    ELSE IF (MODELGS.EQ.6) THEN
        ...
        !GSDIVA = EXP(SF * SOILMOISTURE) * FSOIL * (1.0 + G1 / VPDG**(0.5)) / CS
    END IF
    ...
    END IF
    PENMON = ET / LHV
    IF (PENMON.LT.0.0) PENMON = 0.0            ! BM 12/05 Should not be negative; but it does!JY02/2019
    ...
    !**********************************************************************
    REAL FUNCTION CALCFSOIL(WEIGHTEDSWP,SWPEXP)
    ...
    USE metcom
    IMPLICIT NONE
    REAL SWPEXP,WEIGHTEDSWP

    CALCFSOIL = EXP(SWPEXP*WEIGHTEDSWP)
    ...
    INTEGER MODELGS,SOILDATA,WSOILMETHOD,ITER
    INTEGER IECO,ITERMAX,NSIDES,VFUN,iday,ihour,IJV
