...
!**********************************************************************
    ...
    IF (ISMAESPA .AND. IOFORMAT .EQ. 0) THEN
        ...
        CALL open_file(trim(out_path)//'swplay.dat', USWPLAY, 'write', 'asc', 'replace')    ! mathias d�cembre 2012
...
!            WRITE (UHRLY,717)
   ...
   ! 717 FORMAT('ELMAX: Canopy maximum leaf transpiration rate (mmol m-2 s-1)')
    ...
    421 FORMAT('qn: soil net radiation                             W m-2')
...
!**********************************************************************
    ...
    IF (IOWATBAL .EQ. 1 .AND. IOFORMAT .EQ. 0) THEN
        ...
        ! write swp by layer
        WRITE (USWPLAY, 521) SOILWP(1:NLAYER) ! mathias d�cembre 2012
    ...
    ELSE IF (IOWATBAL .EQ. 1 .AND. IOFORMAT .EQ. 1) THEN
        ...
        ! write swp by layer
        WRITE (USWPLAY) SOILWP(1:NLAYER) ! mathias d�cembre 2012
...
!**********************************************************************
    ...
    ELSE
        ...
        IF (ELP.GT.0.00) THEN
            ...
            DO IALP = 1,NALPHA
                ALPHA(IALP) = (IALP-0.5)*DALPHA     ! option multi date non entr� pour cas elp >0
            END DO
...
!                     T0,TREF,TMAX,SMD1,SMD2,VK1,VK2,WLEAF,NSIDES,SWPEXP
    ...
    NAMELIST /BBLGS/DATES, G0,G1,D0L,GAMMA,WLEAF,NSIDES, &
                    SMD1,SMD2,WC1,WC2,SWPEXP,DATESWLEAF
    NAMELIST /BBMGS/DATES, G0,G1,GAMMA,VPDMIN,GK,WLEAF,NSIDES,SWPEXP
